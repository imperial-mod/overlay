...
module.exports = {
    ...
    getStats: (player, mode, numberFormatter) => {
		...
		const wins = player.stats["SkyWars"]["wins"] || 0
		const losses = player.stats["SkyWars"]["losses"] || 1
		const wlr = Math.round((wins / losses) * 100) / 100 || wins

		const kills = player.stats["SkyWars"]["kills"] || 0
		const deaths = player.stats["SkyWars"]["deaths"] || 1
